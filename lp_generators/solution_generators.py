...
def generate_beta(variables, constraints, basis_split):
    ...
    primals_in_basis = np.random.choice(
        np.arange(0, variables),
        size=primal_count, replace=False, 
        )
    slacks_in_basis = np.random.choice(
...
def generate_alpha(variables, constraints, frac_violations, beta_param,
                   mean_primal, std_primal, mean_dual, std_dual):
    ...
    # slack values
    dual_alpha_vector = np.random.lognormal(
